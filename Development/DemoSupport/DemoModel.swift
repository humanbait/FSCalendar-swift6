...
public enum DemoFixtures {
    ...
    public static let initialDate = date("2024-02-14")
    public static let minimumDate = date("2020-01-01")
    public static let maximumDate = date("2030-12-31")
    public static func lunarSubtitle(_ date: Date) -> String {
...
public struct DemoState {
    public init(page: Date, scope: String, selection: [Date], events: [String]) {
        self.page = page; self.scope = scope; self.selection = selection; self.events = events
    }
    public let page: Date
...
@MainActor
public protocol CalendarDemoDriver: AnyObject {
